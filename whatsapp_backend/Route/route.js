...
route.get('/User',(req, res) => {
  try {
    ...
    con.query(`select * from users where Email = '${payload.data}'`, (err, rows, fields) => {
      if(err) return res.status(200).json({status:400, message : 'err while query'});
      // console.log('user2',rows)
      res.status(200).json(rows[0]);
    }) 
    // res.status(200).json({id : 1,Name:"Arif",Email:"arifmaniyar0@gmail.com",Status:"Available",online:"2020-09-14 07:51:24"});
  }
  catch(err) {
    res.status(200).json({status:400, message : 'err ehole user'});
  }
})
...
route.post('/ChatUsers', (req, res) => {
  ...
  var id = req.body.id;
  if(!id || id === undefined) return res.status(401).json('id not provided');
...
route.post('/allChat', (req, res) => {
  console.log('usr', req.body)
    con.query(`select id,message,type, time(time) as time,uid from chats where (uid = ${req.body.uid} and cid = ${req.body.cid}) or (uid = ${req.body.cid} and cid = ${req.body.uid})`, (err, rows, fields) => {
